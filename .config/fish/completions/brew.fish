...
function __fish_complete_homebrew_formula
  ...
  for cmd in $arguments
    ...
    if contains -- $cmd edit search install homepage home
      command ls (brew --prefix)/Library/Formula | sed s/\.rb//
      return 0
    end
    
    if contains -- $cmd abv info list link command ls ln rm remove uninstall
      command ls (brew --prefix)/Cellar
      return 0
    end
...
function __fish_complete_brew_has_command
  ...
  for cmd in $arguments
    if contains -- $cmd abv info list link command ls ln rm remove uninstall edit install homepage home prune gen search
      return 0
    end
  end
  return 1
...
function __fish_complete_brew_command
  ...
  if contains -- $cmd abv info list link command ls ln rm remove uninstall edit install homepage home prune gen search up update upgrade outdated cleanup
    return 0
  end
  return 1
end
...
complete -c brew -x -a "abv info list link command ls ln rm remove uninstall edit install homepage home prune gen search up update upgrade outdated cleanup" -n '__fish_complete_brew_no_command'
